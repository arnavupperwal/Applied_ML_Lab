"""Preprocessing script for Experiment 2: Iris Flower Classification."""
import sys
...
def main():
    ...
    input_csv = exp_dir / "Dataset" / "IRIS.csv"
    ...
    # 1. Cleaning: auto-detects and removes 3 duplicate rows
    # 2. Wrangling: Label encodes target 'species' ('Iris-setosa', 'Iris-versicolor', 'Iris-virginica' -> 0, 1, 2)
    # 3. Normalization: StandardScaler for numerical flower measurement features
    ...
    training_prefix = exp_dir / "Dataset" / "iris"
    ...
    (dataset_dir / "iris_train.csv").replace(dataset_dir / "Training" / "train.csv")
    (dataset_dir / "iris_val.csv").replace(dataset_dir / "Validation" / "val.csv")
    (dataset_dir / "iris_test.csv").replace(dataset_dir / "Test" / "test.csv")

    print("\nSuccessfully updated EXP_2 split datasets:")
